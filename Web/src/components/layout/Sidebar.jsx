...
export const Sidebar = ({ mobileOpen, setMobileOpen }) => {
  ...
  return (
    <>
      {mobileOpen && (
        <div className="sidebar-backdrop mobile-only" onClick={() => setMobileOpen && setMobileOpen(false)} />
      )}
      <aside className={`w-64 bg-restaurant-dark text-white h-screen fixed left-0 top-0 flex flex-col z-50 ${mobileOpen ? 'is-mobile-open' : ''}`}>
        {/* Header with official logo */}
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          <Link to="/" aria-label="Just Enough Home" onClick={() => setMobileOpen && setMobileOpen(false)}>
        ...
        <div className="p-4 border-t border-white/10">
          <Link
            to="/login"
            onClick={() => setMobileOpen && setMobileOpen(false)}
            className="flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-white w-full text-left transition-colors text-sm font-medium"
          >
            <LogOut size={19} />
            <span>Logout</span>
          </Link>
