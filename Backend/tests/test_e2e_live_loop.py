...
def test_full_vertical_operational_e2e_loop():
    login_res = client.post(
        '/api/v1/auth/login',
        data={'username': 'admin@justenough.local', 'password': 'AdminSecret123!'}
    )
    ...
    # 4b. Verify immutable audit log
    audit_res = client.get('/api/v1/recommendations/overrides/audit', headers=headers)
    assert audit_res.status_code == 200
    audits = audit_res.json()['data']
    assert any(a['entity_id'] == rec_id and a['action'] == 'OVERRIDE_RECOMMENDATION' for a in audits)
